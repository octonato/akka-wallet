...
public class TransferWorkflow extends Workflow<TransferWorkflowState> {
  ...
  public final static String prefix = "tw:";
    ...
    var initiateTransfer =
      step(INITIATE_TRANSFER)
        .asyncCall(() -> {
          ...
          logger.info("Transfer [{}]: withdrawing [{}] to wallet [{}]", transferId, amount, fromWallet);
          var withdrawRes = componentClient
          ...
          logger.info("Transfer [{}]: depositing [{}] to wallet [{}]", transferId, amount, toWallet);
          var depositRes = componentClient
            .forEventSourcedEntity(toWallet)
            .method(WalletEntity::deposit)
            .invokeAsync(new DepositCommand(amount, transferId));

          return CompletableFuture
            .allOf(withdrawRes.toCompletableFuture(), depositRes.toCompletableFuture())
    ...
    var execute =
      step(EXECUTE)
        .asyncCall(() -> {
          ...
          return CompletableFuture
            .allOf(executeFromWallet.toCompletableFuture(), executeToWallet.toCompletableFuture())
            .thenApply(__ -> Done.getInstance());
        })
        .andThen(Done.class, done ->
          effects()
            .updateState(currentState().completed())
            .end()
        );
    ...
    var cancel =
      step(CANCEL)
        .asyncCall(() -> {
          ...
          return CompletableFuture
            .allOf(cancelFromWallet, cancelToWallet)
            .thenApply(__ -> Done.getInstance());
        })
        .andThen(Done.class, done ->
          effects()
  ...
  private CompletableFuture<Done> cancelTransfer(String walletId) {
    ...
    return componentClient
      ...
      .invokeAsync(transferId).toCompletableFuture();
  }
